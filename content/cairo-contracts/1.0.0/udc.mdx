...
Achieving this type of deployment necessitates that the origin sets `from_zero` to `false` in the [deploy_contract](/api/udc.mdx#UniversalDeployer-deploy_contract) call.
...
* `deployContract` method is replaced with the snake_case [deploy_contract](/api/udc.mdx#UniversalDeployer-deploy_contract).
...
* `unique` parameter is replaced with `from_zero` in both the `deploy_contract` method and [ContractDeployed](/api/udc.mdx#IUniversalDeployer-ContractDeployed) event.
...
They include the generic [calculate_contract_address_from_deploy_syscall](/api/utilities.mdx#deployments-calculate_contract_address_from_deploy_syscall) as well as the UDC-specific [calculate_contract_address_from_udc](/api/utilities.mdx#deployments-calculate_contract_address_from_udc).
Check out the [deployments](/api/utilities.mdx#deployments) for more information.
