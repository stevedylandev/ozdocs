...
description: Migrating ERC165 to SRC5
---
...
The contract should include the new introspection mechanism, [SRC5Component](/api/introspection.adoc#SRC5Component).
It should also include the [InitializableComponent](/api/security.adoc#InitializableComponent) which will be used in the `migrate` function.
Here’s the setup:
...
For this example, let’s say that this contract supports the [IERC721](/api/erc721.adoc#IERC721) and [IERC721Metadata](/api/erc721.adoc#IERC721Metadata) interfaces.
...
Since the new contract integrates `SRC5Component`, it can leverage SRC5’s [register_interface](/api/introspection.adoc#SRC5Component-register_interface) function to register the supported interfaces.
...
Since the `migrate` function will be publicly callable, it should include some sort of [Access Control](/access.adoc) so that only permitted addresses can execute the migration.
