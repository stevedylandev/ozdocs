...
to implement the [VotesComponent](api/governance.adoc#VotesComponent) extension. This extension will keep track of historical balances so that voting power
...
We will initially build a Governor without a timelock. The core logic is given by the [GovernorComponent](api/governance.adoc#GovernorComponent), but we
...
***For 1)*** we will use the {GovernorVotes} extension, which hooks to an [IVotes](api/governance.adoc#IVotes) instance to determine the voting power
...
***For 2)*** we will use {GovernorVotesQuorumFraction}. This works together with the [IVotes](api/governance.adoc#IVotes) instance to define the quorum as a
...
with a decision before it is executed. We will use OpenZeppelin’s [TimelockController](governance/timelock.adoc) in combination with the
...
TimelockController uses an [AccessControl](access.adoc#role_based_accesscontrol) setup that we need to understand in order to set up roles.
