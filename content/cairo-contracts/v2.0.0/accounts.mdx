...
<Callout>
For detailed information on the usage and implementation check the [API Reference](/api/account.adoc) section.
</Callout>
...
4. `\__validate_deploy__` optional entrypoint similar to `\__validate__` but meant for [counterfactual deployments](/guides/deployment.adoc).
...
OpenZeppelin Contracts for Cairo provides [AccountComponent](/api/account.adoc#AccountComponent) for implementing this validation scheme.

### Usage

Constructing an account contract requires integrating both [AccountComponent](/api/account.adoc#AccountComponent) and [SRC5Component](/api/introspection.adoc#SRC5Component). The contract should also set up the constructor to initialize the public key that will be used as the account’s signer. Here’s an example of a basic contract:
...
For this the [EthAccountComponent](/api/account.adoc#EthAccountComponent) must be used.

### Usage

Constructing a secp256k1 account contract also requires integrating both [EthAccountComponent](/api/account.adoc#EthAccountComponent) and [SRC5Component](/api/introspection.adoc#SRC5Component).
...
The former can be easily done with the [Universal Deployer Contract (UDC)](udc.adoc), a contract that
...
`\__validate_deploy__`. Check the [counterfactual deployments](/guides/deployment.adoc) guide to learn how.
