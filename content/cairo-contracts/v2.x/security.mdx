...
The [Initializable](api/security.adoc#InitializableComponent) component provides a simple mechanism that mimics
...
The [Pausable](api/security.adoc#PausableComponent) component allows contracts to implement an emergency stop mechanism.
...
For methods that should be available only when paused or not, insert calls to `[assert_paused](api/security.adoc#PausableComponent-assert_paused)` and `[assert_not_paused](api/security.adoc#PausableComponent-assert_not_paused)`
respectively.

### Usage

For example (using the [Ownable](api/access.adoc#OwnableComponent) component for access control):
...
Since Cairo does not support modifiers like Solidity, the [ReentrancyGuard](api/security.adoc#ReentrancyGuardComponent)
component exposes two methods `[start](api/security.adoc#ReentrancyGuardComponent-start)` and `[end](api/security.adoc#ReentrancyGuardComponent-end)` to protect functions against reentrancy attacks.
