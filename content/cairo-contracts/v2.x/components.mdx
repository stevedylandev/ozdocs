...
The `path` argument should be the imported component itself (in this case, [InitializableComponent](/security.adoc#initializable)).
...
Components also separate granular implementations from [mixin](/components.adoc#mixins) implementations.
The API documentation design reflects these groupings.
See [ERC20Component](/api/erc20.adoc#ERC20Component) as an example which includes:
...
Let’s look at how a contract would integrate [OwnableComponent](/api/access.adoc#OwnableComponent):
...
Here’s an example of how to use the Immutable Component Config pattern with the [ERC2981Component](/api/token_common.adoc#ERC2981Component):
...
For instance, [AccessControlComponent](/api/access.adoc#AccessControlComponent) depends on [SRC5Component](/api/introspection.adoc#SRC5Component).
...
Next, the contract includes the [ERC20Component](/api/erc20.adoc#ERC20Component) implementations; however, `ERC20Impl` and `ERC20CamelOnlyImpl` are **not** embedded.
