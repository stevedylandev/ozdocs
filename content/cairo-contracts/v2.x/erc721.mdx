...
The following interface represents the full ABI of the Contracts for Cairo [ERC721Component](/api/erc721.adoc#ERC721Component).
The interface includes the [IERC721](/api/erc721.adoc#IERC721) standard interface and the optional [IERC721Metadata](/api/erc721.adoc#IERC721Metadata) interface.
...
See the [Introspection](introspection.adoc) docs.
...
* ERC721 utilizes [SRC5](introspection.adoc#src5) to declare and query interface support on Starknet as opposed to Ethereum’s [EIP165](https://eips.ethereum.org/EIPS/eip-165).
...
This library includes [transfer_from](api/erc721.adoc#IERC721-transfer_from) and [safe_transfer_from](api/erc721.adoc#IERC721-safe_transfer_from) to transfer NFTs.
...
The recipient contract must also implement the [SRC5](introspection.adoc#src5) interface which, as described earlier, supports interface introspection.
...
Implementing the `IERC721Receiver` interface exposes the [on_erc721_received](api/erc721.adoc#IERC721Receiver-on_erc721_received) method.
When safe methods such as [safe_transfer_from](api/erc721.adoc#IERC721-safe_transfer_from) and [safe_mint](api/erc721.adoc#ERC721-safe_mint) are called, they invoke the recipient contract’s `on_erc721_received` method which **must** return the [IERC721Receiver interface ID](/api/erc721.adoc#IERC721Receiver).
...
ERC721Component now stores only the base URI as a `ByteArray` and the full token URI is returned as the `ByteArray` concatenation of the base URI and the token ID through the [token_uri](/api/erc721.adoc#IERC721Metadata-token_uri) method.
