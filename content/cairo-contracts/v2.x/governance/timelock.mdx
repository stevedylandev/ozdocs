...
When a proposer calls [schedule](api/governance.adoc#ITimelock-schedule), the `OperationState` moves from `Unset` to `Waiting`.
This starts a timer that must be greater than or equal to the minimum delay.
The timer expires at a timestamp accessible through [get_timestamp](api/governance.adoc#ITimelock-get_timestamp).
Once the timer expires, the `OperationState` automatically moves to the `Ready` state.
At this point, it can be executed.

### Execute

By calling [execute](api/governance.adoc#ITimelock-execute), an executor triggers the operation’s underlying transactions and moves it to the `Done` state. If the operation has a predecessor, the predecessor’s operation must be in the `Done` state for this transaction to succeed.

### Cancel

The [cancel](api/governance.adoc#ITimelock-cancel) function allows cancellers to cancel any pending operations.
...
[TimelockControllerComponent](api/governance.adoc#TimelockControllerComponent) leverages an [AccessControlComponent](api/access.adoc#AccessControlComponent) setup that we need to understand in order to set up roles.
...
The minimum delay of a contract is accessible through [get_min_delay](api/governance.adoc#ITimelock-get_min_delay).

### Usage

Integrating the timelock into a contract requires integrating [TimelockControllerComponent](api/governance.adoc#TimelockControllerComponent) as well as [SRC5Component](api/introspection.adoc#SRC5Component) and [AccessControlComponent](api/access.adoc#AccessControlComponent) as dependencies.
