...
to implement the [VotesComponent](api/governance.mdx#VotesComponent) extension. This extension will keep track of historical balances so that voting power
...
We will initially build a Governor without a timelock. The core logic is given by the [GovernorComponent](api/governance.mdx#GovernorComponent), but we
...
***For 1)*** we will use the GovernorVotes extension, which hooks to an [IVotes](api/governance.mdx#IVotes) instance to determine the voting power
...
***For 2)*** we will use GovernorVotesQuorumFraction. This works together with the [IVotes](api/governance.mdx#IVotes) instance to define the quorum as a
...
with a decision before it is executed. We will use OpenZeppelin’s [TimelockController](governance/timelock.mdx) in combination with the
...
TimelockController uses an [AccessControl](access.mdx#role_based_accesscontrol) setup that we need to understand in order to set up roles.
