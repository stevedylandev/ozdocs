---
title: OpenZeppelin Truffle Upgrades API
description: OpenZeppelin Truffle Upgrades API
---

<Callout type='warn'>
This package is deprecated. The recommended alternative is to use [Hardhat](https://hardhat.org/) along with the [Hardhat Upgrades](hardhat-upgrades.mdx) plugin.
</Callout>

Both `deployProxy` and `upgradeProxy` functions will return instances of [Truffle contracts](https://www.trufflesuite.com/docs/truffle/reference/contract-abstractions), and require Truffle contract classes (retrieved via `artifacts.require`) as arguments. For [beacons](https://docs.openzeppelin.com/contracts/4.x/api/proxy#beacon), `deployBeacon` and `upgradeBeacon` will both return an upgradable beacon instance that can be used with a beacon proxy. All deploy and upgrade functions validate that the implementation contract is upgrade-safe, and will fail otherwise.
...
Creates an [upgradable beacon](https://docs.openzeppelin.com/contracts/4.x/api/proxy#UpgradeableBeacon) given a Truffle contract class to use as implementation, and returns the beacon contract instance.
...
Upgrades an [upgradable beacon](https://docs.openzeppelin.com/contracts/4.x/api/proxy#UpgradeableBeacon) at a specified address to a new implementation contract, and returns the beacon contract instance.
...
Deploys a [proxy admin](https://docs.openzeppelin.com/contracts/4.x/api/proxy#ProxyAdmin) contract and returns its address if one was not already deployed on the current network, or just returns the address of the proxy admin if one was already deployed. Note that this plugin currently only supports using one proxy admin per network.
