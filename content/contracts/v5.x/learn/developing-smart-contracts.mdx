...
To get started with Hardhat we will install it in our [project directory](setting-up-a-node-project.mdx#creating-a-project).
...
<Callout>
If you’re unfamiliar with the `npx` command, check out our [Node project setup guide](setting-up-a-node-project.mdx#using-npx).
</Callout>
...
Reusable modules and libraries are the cornerstone of great software. [**OpenZeppelin Contracts**](contracts::index.mdx) contains lots of useful building blocks for smart contracts to build on. And you can rest easy when building on them: they’ve been the subject of multiple audits, with their security and correctness battle-tested.
...
For example, the [`Ownable`](contracts:api:access.mdx#Ownable) contract marks the deployer account as the contract’s owner, and provides a modifier called `onlyOwner`. When applied to a function, `onlyOwner` will cause all function calls that do not originate from the owner account to revert. Functions to [transfer](contracts:api:access.mdx#Ownable-transferOwnership-address-) and [renounce](contracts:api:access.mdx#Ownable-renounceOwnership--) ownership are also available.
...
The [OpenZeppelin Contracts documentation](contracts::index.mdx) is a great place to learn about developing secure smart contract systems. It features both guides and a detailed API reference: see for example the [Access Control](contracts::access-control.mdx) guide to know more about the `Ownable` contract used in the code sample above.
