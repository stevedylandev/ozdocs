...
description: Preparing for mainnet
---

After [running your project on a testnet](connecting-to-public-test-networks.mdx) for some time without issues, you will want to deploy it to the main Ethereum network (aka _mainnet_). However, the planning for going to mainnet should begin much earlier than your planned release date.
...
An _admin_ (short for _administrator_) account is one that has special privileges in your system. For example, an admin may have the power to [pause](contracts:api:utils.mdx#Pausable) a contract. If such an account were to fall in the hands of a malicious user, they could wreak havoc in your system.
...
A special administrator account in an [OpenZeppelin Upgrades Plugins](upgrades-plugins::index.mdx) project is the account with the power to [_upgrade_](upgrading-smart-contracts.mdx) other contracts. This defaults to the externally owned account used to deploy the contracts: while this is good enough for a local or testnet deployment, in mainnet you need to better secure your contracts. An attacker who gets hold of your upgrade admin account can change any contract in your system!
