...
* [verify(proof, root, leaf)](#ECDSA-recover-bytes32-bytes-[`+recover(hash, signature)+`]
* [toEthSignedMessageHash(hash)](#ECDSA-recover-bytes32-uint8-bytes32-bytes32-[`+recover(hash, v, r, s)+`]
* xref-ECDSA-toEthSignedMessageHash-bytes32)]
...
See pass:normal[[``recover``](#`recover(bytes32-hash,-uint8-v,-bytes32-r,-bytes32-s) → address *internal*`)].
/
...
* xref-MerkleProof-verify-bytes32---bytes32-bytes32)]
