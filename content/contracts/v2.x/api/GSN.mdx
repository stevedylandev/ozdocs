...
description: Gas Station Network (GSN)
---
...
<Callout>
If you’re new to the GSN, head over to our [overview of the system](learn::sending-gasless-transactions.mdx) and basic guide to [creating a GSN-capable contract](ROOT:gsn.mdx).
</Callout>
...
Utilities to make writing [GSN strategies](ROOT:gsn-strategies.mdx) easy are available in GSNRecipient, or you can simply use one of our pre-made strategies:

* GSNRecipientERC20Fee charges the end user for gas costs in an application-specific [ERC20 token](ROOT:tokens.mdx#ERC20)
...
[GSN strategies](ROOT:gsn-strategies.mdx#gsn-strategies) for more
information on how to use the pre-built GSNRecipientSignature and
GSNRecipientERC20Fee, or how to write your own.

**Functions**

* [Penalized(relay, sender, amount)](#GSNRecipient-getHubAddr)
* [TransactionRelayed(relay, from, to, selector, status, charge)](#GSNRecipient-_upgradeRelayHub-address)
...
A [GSN strategy](ROOT:gsn-strategies.mdx#gsn-strategies) that allows relayed transactions through when they are
...
* [constructor(trustedSigner)](#GSNRecipientSignature-constructor)]
...
A [GSN strategy](ROOT:gsn-strategies.mdx#gsn-strategies) that charges transaction fees in a special purpose ERC20
token, which we refer to as the gas payment token. The amount charged is exactly the amount of Ether charged to the
recipient. This means that the token is essentially pegged to the value of Ether.

The distribution strategy of the gas payment token to users is not defined by this contract. It’s a mintable token
whose only minter is the recipient, so the strategy must be implemented in a derived contract, making use of the
internal pass:normal[[``_mint``](#`_mint(address-account,-uint256-amount) *internal*`)] function.

**Functions**

* [constructor(name, symbol)](#GSNRecipientERC20Fee-constructor)]
...
* xref-IRelayHub-TransactionRelayed-address-address-address-bytes4-enum-IRelayHub-RelayCallStatus-uint256)]
* xref-IRelayHub-Penalized-address-address-uint256)]
...
Can only be called by the owner of the relay. After the relay’s `unstakeDelay` has elapsed, pass:normal[[``unstake``](#`unstake(address-relay) *external*`)] will be
callable.
...
Unused balance can only be withdrawn by the contract itself, by calling pass:normal[[``withdraw``](#`withdraw(uint256-amount,-address-payable-dest) *external*`)].
