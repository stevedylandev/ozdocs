...
* [release()](#IERC20-totalSupply)
* [releaseTime()](#IERC20-balanceOf-address)
...
This value changes when pass:normal[[``approve``](#`approve(address-spender,-uint256-amount) → bool *external*`)] or transferFrom are called.
...
a call to pass:normal[[``approve``](#`approve(address-spender,-uint256-amount) → bool *external*`)]. `value` is the new allowance.
...
that a supply mechanism has to be added in a derived contract using pass:normal[[``_mint``](#`_mint(address-account,-uint256-amount) *internal*`)].
...
* [constructor(name_, symbol_)](#ERC20-constructor)]
* [name()](#ERC20-name)]
* [symbol()](#ERC20-symbol)]
...
Sets the values for pass:normal[[``name``](#`name() → string *public*`)] and pass:normal[[``symbol``](#`symbol() → string *public*`)], initializes pass:normal[[``decimals``](#`decimals() → uint8 *public*`)] with
a default value of 18.

To select a different value for pass:normal[[``decimals``](#`decimals() → uint8 *public*`)], use _setupDecimals.
...
This is an alternative to pass:normal[[``approve``](#`approve(address-spender,-uint256-amount) → bool *public*`)] that can be used as a mitigation for
...
This is internal function is equivalent to pass:normal[[``transfer``](#`transfer(address-recipient,-uint256-amount) → bool *public*`)], and can be used to
...
Sets pass:normal[[``decimals``](#`decimals() → uint8 *public*`)] to a value other than the default one of 18.

<Callout type='warn'>
This function should only be called from the constructor. Most
</Callout>
applications that interact with token contracts will not expect
pass:normal[[``decimals``](#`decimals() → uint8 *public*`)] to ever change, and may work incorrectly if it does.
...
Snapshots are created by the internal pass:normal[[``_snapshot``](#`_snapshot() → uint256 *internal*`)] function, which will emit the Snapshot event and return a
...
pass:normal[[``_snapshot``](#`_snapshot() → uint256 *internal*`)] is `internal` and you have to decide how to expose it externally. Its usage may be restricted to a
set of accounts, for example using AccessControl, or it may be open to the public.



While an open way of calling pass:normal[[``_snapshot``](#`_snapshot() → uint256 *internal*`)] is required for certain trust minimization mechanisms such as forking,
...
Emitted by pass:normal[[``_snapshot``](#`_snapshot() → uint256 *internal*`)] when a snapshot identified by `id` is created.
...
* [constructor(cap_)](#ERC20Capped-constructor)]
* [cap()](#ERC20Capped-cap)]
...
* [constructor(token_, beneficiary_, releaseTime_)](#TokenTimelock-constructor)]
* [token()](#TokenTimelock-token)]
* [beneficiary()](#TokenTimelock-beneficiary)]
* xref-TokenTimelock-releaseTime)]
