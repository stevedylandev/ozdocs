...
* [toString(value)](#Pausable-whenNotPaused)
* [toInt256(value)](#Pausable-whenPaused)
...
Returns the address where a contract will be stored if deployed via pass:normal[[``deploy``](#`deploy(uint256-amount,-bytes32-salt,-bytes-bytecode) → address *internal*`)]. Any change in the
`bytecodeHash` or `salt` will result in a new destination address.

#### `computeAddress(bytes32 salt, bytes32 bytecodeHash, address deployer) → address *internal*`

Returns the address where a contract will be stored if deployed via pass:normal[[``deploy``](#`deploy(uint256-amount,-bytes32-salt,-bytes-bytecode) → address *internal*`)] from a contract located at
...
* `index` must be strictly less than pass:normal[[``length``](#`length(struct-enumerablemap.uinttoaddressmap-map) → uint256 *internal*`)].
...
Same as pass:normal[[``get``](#`get(struct-enumerablemap.uinttoaddressmap-map,-uint256-key,-string-errormessage) → address *internal*`)], with a custom error message when `key` is not in the map.
...
* `index` must be strictly less than pass:normal[[``length``](#`length(struct-enumerableset.uintset-set) → uint256 *internal*`)].
...
* xref-SafeCast-toInt256-uint256)]
