...
To manage all of the different combinations and flavors of crowdsales, Contracts provides a highly configurable [`Crowdsale`](contracts/v5.x/api/crowdsale.mdx#Crowdsale) base contract that can be combined with various other functionalities to construct a bespoke crowdsale.
...
If you want to issue someone <q>1 TKN` for every `1 ETH</q>, and your decimals are 18, your rate is `1`. This is because what’s actually happening with the math is that the contract sees a user send `10^18 wei`, not `1 ETH`. Then it uses your rate of 1 to calculate `TKNbits = rate * wei`, or `1 * 10^18`, which is still `10^18`. And because your decimals are 18, this is displayed as `1 TKN`.
...
* [`MintedCrowdsale`](contracts/v5.x/api/crowdsale.mdx#MintedCrowdsale) — The `Crowdsale` mints tokens when a purchase is made.
* [`AllowanceCrowdsale`](contracts/v5.x/api/crowdsale.mdx#AllowanceCrowdsale) — The `Crowdsale` is granted an allowance to another wallet (like a Multisig) that already owns the tokens to be sold in the crowdsale.
...
To use a [`MintedCrowdsale`](contracts/v5.x/api/crowdsale.mdx#MintedCrowdsale), your token must also be a [`ERC20Mintable`](contracts/v5.x/api/token/ERC20.mdx#ERC20Mintable) token that the crowdsale has permission to mint from. This can look like:
...
Use an [`AllowanceCrowdsale`](contracts/v5.x/api/crowdsale.mdx#AllowanceCrowdsale) to send tokens from another wallet to the participants of the crowdsale. In order for this to work, the source wallet must give the crowdsale an allowance via the ERC20 [`approve`](contracts/v5.x/api/token/ERC20.mdx#IERC20-approve-address-uint256-) method.
...
* [`CappedCrowdsale`](contracts/v5.x/api/crowdsale.mdx#CappedCrowdsale) — adds a cap to your crowdsale, invalidating any purchases that would exceed that cap
* [`IndividuallyCappedCrowdsale`](contracts/v5.x/api/crowdsale.mdx#IndividuallyCappedCrowdsale) — caps an individual’s contributions.
* [`WhitelistCrowdsale`](contracts/v5.x/api/crowdsale.mdx#WhitelistCrowdsale) — only allow whitelisted participants to purchase tokens. this is useful for putting your KYC / AML whitelist on-chain!
* [`TimedCrowdsale`](contracts/v5.x/api/crowdsale.mdx#TimedCrowdsale) — adds an [`openingTime`](contracts/v5.x/api/crowdsale.mdx#TimedCrowdsale-openingTime--) and [`closingTime`](contracts/v5.x/api/Crowdsale.mdx#TimedCrowdsale-closingTime--) to your crowdsale
...
The [`PostDeliveryCrowdsale`](contracts/v5.x/api/crowdsale.mdx#PostDeliveryCrowdsale), as its name implies, distributes tokens after the crowdsale has finished, letting users call [`withdrawTokens`](contracts/v5.x/api/crowdsale.mdx#PostDeliveryCrowdsale-withdrawTokens_address-) in order to claim the tokens they’ve purchased.
...
The [`RefundableCrowdsale`](contracts/v5.x/api/crowdsale.mdx#RefundableCrowdsale) offers to refund users if a minimum goal is not reached. If the goal is not reached, the users can [`claimRefund`](contracts/v5.x/api/crowdsale.mdx#RefundableCrowdsale-claimRefund-address-payable-) to get their Ether back.
