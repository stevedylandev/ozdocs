...
The [`ERC721Full`](contracts/v5.x/api/token/ERC721.mdx#ERC721Full) contract includes all standard extensions, and is probably the one you want to use. In particular, it includes [`ERC721Metadata`](contracts/v5.x/api/token/ERC721.mdx#ERC721Metadata), which provides the [`_setTokenURI`](contracts/v5.x/api/token/ERC721.mdx#ERC721Metadata-_setTokenURI-uint256-string-) method we use to store an item’s metadata.
