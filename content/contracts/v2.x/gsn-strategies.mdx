...
As we have seen in the [GSN Guide](gsn.mdx), in order to be GSN enabled, your contracts need to extend from [`GSNRecipient`](contracts/v5.x/api/GSN.mdx#GSNRecipient).
...
The actual user’s `msg.sender` and `msg.data` can be obtained safely via [`_msgSender()`](contracts/v5.x/api/GSN.mdx#GSNRecipient-_msgSender--) and [`_msgData()`](contracts/v5.x/api/GSN.mdx#GSNRecipient-_msgData--) of [`GSNRecipient`](contracts/v5.x/api/GSN.mdx#GSNRecipient).
...
The base [`GSNRecipient`](contracts/v5.x/api/GSN.mdx#GSNRecipient) contract doesn’t include a strategy, so you must either use one of the pre-built ones or write your own. We will first go over using the included strategies: [`GSNRecipientSignature`](contracts/v5.x/api/GSN.mdx#GSNRecipientSignature) and [`GSNRecipientERC20Fee`](contracts/v5.x/api/GSN.mdx#GSNRecipientERC20Fee).

## `GSNRecipientSignature`

[`GSNRecipientSignature`](contracts/v5.x/api/GSN.mdx#GSNRecipientSignature) lets users relay calls via the GSN to your recipient contract (charging you for it) if they can prove that an account you trust approved them to do so. The way they do this is via a _signature_.
...
[`GSNRecipientERC20Fee`](contracts/v5.x/api/GSN.mdx#GSNRecipientERC20Fee) is a bit more complex (but don’t worry, it has already been written for you!). Unlike `GSNRecipientSignature`, `GSNRecipientERC20Fee` doesn’t require any off-chain services.
...
`GSNRecipientERC20Fee` has an internal [`_mint`](contracts/v5.x/api/GSN.mdx#GSNRecipientERC20Fee-_mint-address-uint256-) function. Firstly, you need to setup a way to call it (e.g. add a public function with some form of [access control](access-control.mdx) such as [`onlyMinter`](contracts/v5.x/api/access.mdx#MinterRole-onlyMinter--)).
...
See [the code for `GSNRecipientERC20Fee`](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v2.4.0/contracts/GSN/GSNRecipientERC20Fee.sol) as an example implementation.
