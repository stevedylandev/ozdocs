...
description: Creating ERC20 Supply
---
...
The standard interface implemented by tokens built on Ethereum is called ERC20, and Contracts includes a widely used implementation of it: the aptly named [`ERC20`](contracts/v5.x/api/token/ERC20.mdx) contract. This contract, like the standard itself, is quite simple and bare-bones. In fact, if you try deploy an instance of `ERC20` as-is it will be quite literally useless... it will have no supply! What use is a token with no supply?
...
Starting with Contracts v2 this pattern is not only discouraged, but disallowed. The variables `totalSupply` and `balances` are now private implementation details of `ERC20`, and you can’t directly write to them. Instead, there is an internal [`_mint`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_mint-address-uint256-) function that will do exactly this:
...
The internal [`_mint`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_mint-address-uint256-) function is the key building block that allows us to write ERC20 extensions that implement a supply mechanism.
...
There is one supply mechanism already included in Contracts: [`ERC20Mintable`](contracts/v5.x/api/token/ERC20.mdx#ERC20Mintable). This is a generic mechanism in which a set of accounts is assigned the `minter` role, granting them the permission to call a [`mint`](contracts/v5.x/api/token/ERC20.mdx#ERC20Mintable-mint-address-uint256-) function, an external version of `_mint`.
...
Additionally to `_mint`, `ERC20` provides other internal functions that can be used or extended, such as [`_transfer`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_transfer-address-address-uint256-). This function implements token transfers and is used by `ERC20`, so it can be used to trigger functionality automatically. This is something that can’t be done with the `ERC20Mintable` approach.
