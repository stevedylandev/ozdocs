...
OpenZeppelin Contracts has some ``library``s: most are in the [Utils](contracts/v3.x/api/utils) directory.
...
For example, imagine you want to change [`AccessControl`](contracts/v3.x/api/access#AccessControl) so that [`revokeRole`](contracts/v3.x/api/access#AccessControl-revokeRole-bytes32-address-) can no longer be called. This can be achieved using overrides:
...
Here is a modified version of [`AccessControl`](contracts/v3.x/api/access#AccessControl) where [`revokeRole`](contracts/v3.x/api/access#AccessControl-revokeRole-bytes32-address-) cannot be used to revoke the `DEFAULT_ADMIN_ROLE`:
...
Here’s how you would implement the `IERC721Receiver` pattern in `ERC20`, using the [`_beforeTokenTransfer`](contracts/v3.x/api/token/ERC20#ERC20-_beforeTokenTransfer-address-address-uint256-) hook:
...
So far, the only available hook is `_beforeTransferHook`, in all of [`ERC20`](contracts/v3.x/api/token/ERC20#ERC20-_beforeTokenTransfer-address-address-uint256-), [`ERC721`](contracts/v3.x/api/token/ERC721#ERC721-_beforeTokenTransfer-address-address-uint256-), [`ERC777`](contracts/v3.x/api/token/ERC777#ERC777-_beforeTokenTransfer-address-address-address-uint256-) and [`ERC1155`](contracts/v3.x/api/token/ERC1155#ERC1155-_beforeTokenTransfer-address-address-address-uint256---uint256---bytes-). If you have ideas for new hooks, let us know!
...
2. ***Always*** call the parent’s hook in your override using `super`. This will make sure all hooks in the inheritance tree are called: contracts like [`ERC20Pausable`](contracts/v3.x/api/token/ERC20#ERC20Pausable) rely on this behavior.
