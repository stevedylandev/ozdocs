...
The internal [`_mint`](contracts/v3.x/api/token/ERC20#ERC20-_mint-address-uint256-) function is the key building block that allows us to write ERC20 extensions that implement a supply mechanism.
...
So far our supply mechanisms were triggered manually, but `ERC20` also allows us to extend the core functionality of the token through the [`_beforeTokenTransfer`](contracts/v3.x/api/token/ERC20#ERC20-_beforeTokenTransfer-address-address-uint256-) hook (see [Using Hooks](extending-contracts#using-hooks)).
