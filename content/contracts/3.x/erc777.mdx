...
We will replicate the `GLD` example of the [ERC20 guide](erc20#constructing-an-erc20-token-contract), this time using ERC777. As always, check out the [`API reference`](contracts/v3.x/api/token/ERC777) to learn more about the details of each function.
...
That’s it for a basic token contract! We can now deploy it, and use the same [`balanceOf`](contracts/v3.x/api/token/ERC777#IERC777-balanceOf-address-) method to query the deployer’s balance:
...
A key difference when using [`send`](contracts/v3.x/api/token/ERC777#ERC777-send-address-uint256-bytes-) is that token transfers to other contracts may revert with the following message:
