...
<Callout>
ERC1155 draws ideas from all of [ERC20](erc20.mdx), [ERC721](721.mdx), and [ERC777](erc777.mdx). If you’re unfamiliar with those standards, head to their guides before moving on.
</Callout>

## Multi Token Standard

The distinctive feature of ERC1155 is that it uses a single smart contract to represent multiple tokens at once. This is why its [`balanceOf`](contracts/v3.x/api/token/ERC1155.mdx#IERC1155-balanceOf-address-uint256-) function differs from ERC20’s and ERC777’s: it has an additional `id` argument for the identifier of the token that you want to query the balance of.
...
Because all state is held in a single contract, it is possible to operate over multiple tokens in a single transaction very efficiently. The standard provides two functions, [`balanceOfBatch`](contracts/v3.x/api/token/ERC1155.mdx#IERC1155-balanceOfBatch-address---uint256---) and [`safeBatchTransferFrom`](contracts/v3.x/api/token/ERC1155.mdx#IERC1155-safeBatchTransferFrom-address-address-uint256---uint256---bytes-), that make querying multiple balances and transferring multiple tokens simpler and less gas-intensive.

In the spirit of the standard, we’ve also included batch operations in the non-standard functions, such as [`_mintBatch`](contracts/v3.x/api/token/ERC1155.mdx#ERC1155-_mintBatch-address-uint256---uint256---bytes-).
...
The [`ERC1155`](contracts/v3.x/api/token/ERC1155.mdx#ERC1155) contract includes the optional extension [`IERC1155MetadataURI`](contracts/v3.x/api/token/ERC1155.mdx#IERC1155MetadataURI). That’s where the [`uri`](contracts/v3.x/api/token/ERC1155.mdx#IERC1155MetadataURI-uri-uint256-) function comes from: we use it to retrieve the metadata uri.
...
A key difference when using [`safeTransferFrom`](contracts/v3.x/api/token/ERC1155.mdx#IERC1155-safeTransferFrom-address-address-uint256-uint256-bytes-) is that token transfers to other contracts may revert with the following message:
