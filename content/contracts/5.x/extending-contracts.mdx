...
OpenZeppelin Contracts has some ``library``s: most are in the [Utils](contracts/v5.x/api/utils) directory.
</Callout>
...
For example, imagine you want to change [`AccessControl`](contracts/v5.x/api/access#AccessControl) so that [`revokeRole`](contracts/v5.x/api/access#AccessControl-revokeRole-bytes32-address-) can no longer be called. This can be achieved using overrides:
...
Here is a modified version of [`AccessControl`](contracts/v5.x/api/access#AccessControl) where [`revokeRole`](contracts/v5.x/api/access#AccessControl-revokeRole-bytes32-address-) cannot be used to revoke the `DEFAULT_ADMIN_ROLE`:
...
<Callout>
The same rule is implemented and extended in [`AccessControlDefaultAdminRules`](contracts/v5.x/api/access#AccessControlDefaultAdminRules), an extension that also adds enforced security measures for the `DEFAULT_ADMIN_ROLE`.
</Callout>
