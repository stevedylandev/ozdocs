...
* [`Heap`](/contracts/5.x/api/utils#Heap.sol): A [binary heap](https://en.wikipedia.org/wiki/Binary_heap) to store elements with priority defined by a compartor function.
...
OpenZeppelin Contracts implements a Heap data structure with the properties of a binary heap. The heap uses the [`lt`](/contracts/5.x/api/utils/cryptography#Comparators-lt-uint256-uint256-) function by default but allows to customize its comparator.
