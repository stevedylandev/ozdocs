...
In this guide, we will see how we can use OpenZeppelin libraries such as [Network JS](network-js::index) and the [hot-loader](https://github.com/OpenZeppelin/solidity-loader) to build your dapp, as well as how the [Starter Kits](starter-kits::index) bind all pieces of a dapp together:
...
<Callout>
You can easily [enable meta-transaction support](sending-gasless-transactions) in your dapp when using Network JS by [adding a `gsn` option](network-js::api#webcontextoptions) to the React hook.
</Callout>

To learn more, head to our step-by-step tutorial on [Building a Dapp from Scratch](network-js::building-a-dapp-from-scratch) using the Networks library.

## Hot-loading Contracts

Now that we have a working connection to the Ethereum network, we can start interacting with the contracts we have coded and [deployed using the OpenZeppelin CLI](deploying-and-interacting).

While we _could_ create a regular `web3` contract instance [from ABI and address](contract-loader::index), we will instead leverage [contract upgrades](upgrading-smart-contracts) to hot-load our contracts.
...
To kickstart this process, we have built the [**OpenZeppelin Starter Kits**](starter-kits::index). Starter Kits are preconfigured dapps with several OpenZeppelin libraries, Infura connections, and [Rimble UI components](https://github.com/ConsenSys/rimble-ui), ready to start developing right away.
...
If you want to learn more about using Starter Kits, `unpack` the [`tutorial`](starter-kits::tutorial) kit instead of `starter`: it will guide you through the process of deploying and interacting with contracts from a dapp. You can also check out the [list of all starter kits](starter-kits::list) available to unpack.
