...
The standard interface implemented by tokens built on Ethereum is called ERC20, and Contracts includes a widely used implementation of it: the aptly named [`ERC20`](contracts/v5.x/api/token/ERC20.mdx) contract. This contract, like the standard itself, is quite simple and bare-bones. In fact, if you try to deploy an instance of `ERC20` as-is it will be quite literally useless... it will have no supply! What use is a token with no supply?
...
<Callout>
To learn more about roles and permissioned systems, head to our [Access Control guide](access-control.mdx).
</Callout>

## Automating the Reward

So far our supply mechanisms were triggered manually, but `ERC20` also allows us to extend the core functionality of the token through the [`_beforeTokenTransfer`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_beforeTokenTransfer-address-address-uint256-) hook (see [Using Hooks](extending-contracts.mdx#using-hooks)).
