...
OpenZeppelin provides the [`CrossChainEnabled`](contracts/v5.x/api/crosschain.mdx#CrossChainEnabled) abstract contract, that includes dedicated internal functions.
...
In order to do that, we will start by adding [`CrossChainEnabled`](contracts/v5.x/api/crosschain.mdx#CrossChainEnabled) to our contract. You will notice that the contract is now abstract. This is because `CrossChainEnabled` is an abstract contract: it is not tied to any particular chain and it deals with cross-chain interactions in an abstract way. This is what enables us to easily reuse the code for different chains. We will specialize it later by inheriting from a chain-specific implementation of the abstraction.
