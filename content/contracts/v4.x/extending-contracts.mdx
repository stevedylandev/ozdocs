...
OpenZeppelin Contracts has some ``library``s: most are in the [Utils](contracts/v5.x/api/utils.mdx) directory.
...
For example, imagine you want to change [`AccessControl`](contracts/v5.x/api/access.mdx#AccessControl) so that [`revokeRole`](contracts/v5.x/api/access.mdx#AccessControl-revokeRole-bytes32-address-) can no longer be called. This can be achieved using overrides:
...
Here is a modified version of [`AccessControl`](contracts/v5.x/api/access.mdx#AccessControl) where [`revokeRole`](contracts/v5.x/api/access.mdx#AccessControl-revokeRole-bytes32-address-) cannot be used to revoke the `DEFAULT_ADMIN_ROLE`:
...
<Callout>
The same rule is implemented and extended in [`AccessControlDefaultAdminRules`](contracts/v5.x/api/access.mdx#AccessControlDefaultAdminRules), an extension that also adds enforced security measures for the `DEFAULT_ADMIN_ROLE`.
</Callout>
...
For example, consider implementing safe [`ERC20`](contracts/v5.x/api/token/ERC20.mdx#ERC20) transfers in the style of [`IERC721Receiver`](contracts/v5.x/api/token/ERC721.mdx#IERC721Receiver). You may think overriding [`transfer`](contracts/v5.x/api/token/ERC20.mdx#ERC20-transfer-address-uint256-) and [`transferFrom`](contracts/v5.x/api/token/ERC20.mdx#ERC20-transferFrom-address-address-uint256-) would be enough, but what about  [`_transfer`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_transfer-address-address-uint256-) and [`_mint`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_mint-address-uint256-)? To prevent you from having to deal with these details, we introduced ***hooks***.
...
Here’s how you would implement the `IERC721Receiver` pattern in `ERC20`, using the [`_beforeTokenTransfer`](contracts/v5.x/api/token/ERC20.mdx#ERC20-_beforeTokenTransfer-address-address-uint256-) hook:
...
2. ***Always*** call the parent’s hook in your override using `super`. This will make sure all hooks in the inheritance tree are called: contracts like [`ERC20Pausable`](contracts/v5.x/api/token/ERC20.mdx#ERC20Pausable) rely on this behavior.
