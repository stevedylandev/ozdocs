...
The [`ERC721URIStorage`](contracts/v4.x/api/token/ERC721.mdx#ERC721URIStorage) contract is an implementation of ERC721 that includes the metadata standard extensions ([`IERC721Metadata`](contracts/v4.x/api/token/ERC721.mdx#IERC721Metadata)) as well as a mechanism for per-token metadata. That’s where the [`_setTokenURI`](contracts/v4.x/api/token/ERC721.mdx#ERC721-_setTokenURI-uint256-string-) method comes from: we use it to store an item’s metadata.
