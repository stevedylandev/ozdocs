...
The OpenZeppelin Contracts provide a ton of useful utilities that you can use in your project. For a complete list, check out the [API Reference](contracts/v5.x/api/utils.mdx).
Here are some of the more popular ones.

## Cryptography

<mark>#</mark> Checking Signatures On-Chain
...
<mark>#</mark># Ethereum Signatures (secp256k1)

[`ECDSA`](contracts/v5.x/api/utils/cryptography.mdx#ECDSA) provides functions for recovering and managing Ethereum account ECDSA signatures. These are often generated via [`web3.eth.sign`](https://web3js.readthedocs.io/en/v1.7.3/web3-eth.html#sign), and form a 65-byte array (of type `bytes` in Solidity) arranged the following way: `[[v (1)], [r (32)], [s (32)]]`.

The data signer can be recovered with [`ECDSA.recover`](contracts/v5.x/api/utils/cryptography.mdx#ECDSA-recover-bytes32-bytes-), and its address compared to verify the signature. Most wallets will hash the data to sign and add the prefix `\x19Ethereum Signed Message:\n`, so when attempting to recover the signer of an Ethereum signed message hash, you’ll want to use [`toEthSignedMessageHash`](contracts/v5.x/api/utils/cryptography.mdx#MessageHashUtils-toEthSignedMessageHash-bytes32-).
...
<Callout type='warn'>
Getting signature verification right is not trivial: make sure you fully read and understand [`MessageHashUtils`](contracts/v5.x/api/utils/cryptography.mdx#MessageHashUtils)'s and [`ECDSA`](contracts/v5.x/api/utils/cryptography.mdx#ECDSA)'s documentation.
</Callout>

<mark>#</mark># P256 Signatures (secp256r1)
...
To verify a signature using RSA, you can leverage the [`RSA`](contracts/v5.x/api/utils/cryptography.mdx#RSA) library that exposes a method for verifying RSA with the PKCS 1.5 standard:
...
<mark>#</mark> Signature Verification

The [`SignatureChecker`](contracts/v5.x/api/utils/cryptography.mdx#SignatureChecker) library provides a unified interface for verifying signatures from different sources. It seamlessly supports:
...
<mark>#</mark># Basic Signature Verification
...
<mark>#</mark># ERC-1271 Contract Signatures
...
<mark>#</mark># ERC-7913 Extended Signatures
...
<mark>#</mark> Verifying Merkle Proofs
...
[`MerkleProof`](contracts/v5.x/api/utils/cryptography.mdx#MerkleProof) provides:

* [`verify`](contracts/v5.x/api/utils/cryptography.mdx#MerkleProof-verify-bytes32---bytes32-bytes32-) - can prove that some value is part of a [Merkle tree](https://en.wikipedia.org/wiki/Merkle_tree).
* [`multiProofVerify`](contracts/v5.x/api/utils/cryptography.mdx#MerkleProof-multiProofVerify-bytes32-bytes32---bytes32---bool---) - can prove multiple values are part of a Merkle tree.

For an on-chain Merkle Tree, see the [`MerkleTree`](contracts/v5.x/api/utils.mdx#MerkleTree) library.
...
* [`IERC165`](contracts/v5.x/api/utils.mdx#IERC165) — this is the ERC-165 interface that defines [`supportsInterface`](contracts/v5.x/api/utils.mdx#IERC165-supportsInterface-bytes4-). When implementing ERC-165, you’ll conform to this interface.
* [`ERC165`](contracts/v5.x/api/utils.mdx#ERC165) — inherit this contract if you’d like to support interface detection using a lookup table in contract storage. You can register interfaces using [`_registerInterface(bytes4)`](contracts/v5.x/api/utils.mdx#ERC165-_registerInterface-bytes4-): check out example usage as part of the ERC-721 implementation.
* [`ERC165Checker`](contracts/v5.x/api/utils.mdx#ERC165Checker) — ERC165Checker simplifies the process of checking whether or not a contract supports an interface you care about.
* include with `using ERC165Checker for address;`
* [`myAddress._supportsInterface(bytes4)`](contracts/v5.x/api/utils.mdx#ERC165Checker-_supportsInterface-address-bytes4-)
* [`myAddress._supportsAllInterfaces(bytes4[\])`](contracts/v5.x/api/utils.mdx#ERC165Checker-_supportsAllInterfaces-address-bytes4---)
...
Although Solidity already provides math operators (i.e. `+`, `-`, etc.), Contracts includes [`Math`](contracts/v5.x/api/utils.mdx#Math); a set of utilities for dealing with mathematical operators, with support for extra operations (e.g., [`average`](contracts/v5.x/api/utils.mdx#Math-average-uint256-uint256-)) and [`SignedMath`](contracts/v5.x/api/utils.mdx#SignedMath); a library specialized in signed math operations.
...
<Callout>
While working with different data types that might require casting, you can use [`SafeCast`](contracts/v5.x/api/utils.mdx#SafeCast) for type casting with added overflow checks.
</Callout>
...
* [`BitMaps`](contracts/v5.x/api/utils.mdx#BitMaps): Store packed booleans in storage.
* [`Checkpoints`](contracts/v5.x/api/utils.mdx#Checkpoints): Checkpoint values with built-in lookups.
* [`DoubleEndedQueue`](contracts/v5.x/api/utils.mdx#DoubleEndedQueue): Store items in a queue with `pop()` and `queue()` constant time operations.
* [`EnumerableSet`](contracts/v5.x/api/utils.mdx#EnumerableSet): A [set](https://en.wikipedia.org/wiki/Set_(abstract_data_type)) with enumeration capabilities.
* [`EnumerableMap`](contracts/v5.x/api/utils.mdx#EnumerableMap): A `mapping` variant with enumeration capabilities.
* [`MerkleTree`](contracts/v5.x/api/utils.mdx#MerkleTree): An on-chain [Merkle Tree](https://wikipedia.org/wiki/Merkle_Tree) with helper functions.
* [`Heap`](contracts/v5.x/api/utils.mdx#Heap.sol): A [binary heap](https://en.wikipedia.org/wiki/Binary_heap) to store elements with priority defined by a compartor function.
...
<mark>#</mark> Building a Merkle Tree

Building an on-chain Merkle Tree allows developers to keep track of the history of roots in a decentralized manner. For these cases, the [`MerkleTree`](contracts/v5.x/api/utils.mdx#MerkleTree) includes a predefined structure with functions to manipulate the tree (e.g. pushing values or resetting the tree).
...
The library also supports custom hashing functions, which can be passed as an extra parameter to the [`push`](contracts/v5.x/api/utils.mdx#MerkleTree-push-struct-MerkleTree-Bytes32PushTree-bytes32-) and [`setup`](contracts/v5.x/api/utils.mdx#MerkleTree-setup-struct-MerkleTree-Bytes32PushTree-uint8-bytes32-) functions.
...
OpenZeppelin Contracts implements a Heap data structure with the properties of a binary heap. The heap uses the [`lt`](contracts/v5.x/api/utils.mdx#Comparators-lt-uint256-uint256-) function by default but allows to customize its comparator.
...
The [`Packing`](contracts/v5.x/api/utils.mdx#Packing) library is a set of utilities for packing values that fit in 32 bytes. The library includes 3 main functionalities:
...
For those cases, the [`StorageSlot`](contracts/v5.x/api/utils.mdx#StorageSlot) library allows for manipulating storage slots directly.
...
The [`TransientSlot`](contracts/v5.x/api/utils.mdx#TransientSlot) library supports transient storage through user defined value types ([UDVTs](https://docs.soliditylang.org/en/latest/types.html#user-defined-value-types)), which enables the same value types as in Solidity.
...
Users can leverage this standard using the [`SlotDerivation`](contracts/v5.x/api/utils.mdx#SlotDerivation) library.
...
[`Base64`](contracts/v5.x/api/utils.mdx#Base64) util allows you to transform `bytes32` data into its Base64 `string` representation.

This is especially useful for building URL-safe tokenURIs for both [`ERC-721`](contracts/v5.x/api/token/ERC721.mdx#IERC721Metadata-tokenURI-uint256-) or [`ERC-1155`](contracts/v5.x/api/token/ERC1155.mdx#IERC1155MetadataURI-uri-uint256-). This library provides a clever way to serve URL-safe [Data URI](https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs/) compliant strings to serve on-chain data structures.
...
The [`Memory`](contracts/v5.x/api/utils.mdx#Memory) library provides functions for advanced use cases that require granular memory management. A common use case is to avoid unnecessary memory expansion costs when performing repeated operations that allocate memory in a loop. Consider the following example:
...
The [`Time`](contracts/v5.x/api/utils.mdx#Time) library provides helpers for manipulating time-related objects in a type-safe manner. It uses `uint48` for timepoints and `uint32` for durations, helping to reduce gas costs while providing adequate precision.
...
contract MyDelayedContract
    ...
    pdate the delay with `Time`'s safety mechanism
    tion updateDelay(uint32 newDelay) external {
    ...
    et complete delay details including pending changes
    tion getDelayDetails() external view returns (
...
This pattern is used extensively in OpenZeppelin’s [AccessManager](contracts/v5.x/api/access.mdx#AccessManager) for implementing secure time-based access control. For example, when changing an admin delay:
