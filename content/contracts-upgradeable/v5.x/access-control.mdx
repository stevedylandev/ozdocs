...
Here’s a simple example of using `AccessControl` in an [ERC-20 token](erc20.mdx) to define a 'minter' role, which allows accounts that have it create new tokens:
...
Here’s a simple example of an [ERC-20 token](tokens.mdx#ERC20) that defines a `mint` function that is restricted by an [`AccessManager`](contracts/v5.x/api/access.mdx#AccessManager):
