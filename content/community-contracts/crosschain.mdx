...
* On the ***source chain***, the contract implements a standard [`sendMessage`](contracts/v5.x/api/crosschain#AxelarGatewaySource-sendMessage-bytes-bytes-bytes---) function and emits a [`MessageSent`](contracts/v5.x/api/crosschain#AxelarGatewaySource-MessageSent-bytes32-string-string-bytes-bytes---) event to signal that the message should be relayed by the underlying protocol.
* On the ***destination chain***, the gateway receives the message and passes it to the receiver contract by calling the [`receiveMessage`](contracts/v5.x/api/crosschain#ERC7786Receiver-receiveMessage-bytes32-bytes-bytes-) function.

Smart contract developers only need to worry about implementing the [IERC7786GatewaySource](contracts/v5.x/api/crosschain#IERC7786GatewaySource) interface to send a message on the source chain and the [IERC7786GatewaySource](contracts/v5.x/api/crosschain#IERC7786GatewaySource) and [IERC7786Receiver](contracts/v5.x/api/crosschain#IERC7786Receiver) interface to receive such message on the destination chain.
...
<Callout>
Developers can register supported chains and destination gateways using the [`registerChainEquivalence`](contracts/v5.x/api/crosschain#AxelarGatewayBase-registerChainEquivalence-string-string-) and [`registerRemoteGateway`](contracts/v5.x/api/crosschain#AxelarGatewayBase-registerRemoteGateway-string-string-) functions
</Callout>
...
The interface for a source gateway is general enough that it allows wrapping a custom protocol to authenticate messages. Depending on the use case, developers can implement any offchain mechanism to read the standard [`MessageSent`](contracts/v5.x/api/crosschain#IERC7786GatewaySource-MessageSent-bytes32-string-string-bytes-bytes---) event and deliver it to the receiver on the destination chain.
...
To receive a message on a custom smart contract, OpenZeppelin Community Contracts provide an [ERC7786Receiver](contracts/v5.x/api/crosschain#ERC7786Receiver) implementation for developers to inherit. This way your contracts can receive a cross-chain message relayed through a known destination gateway gateway.
...
Aside from the [AxelarGatewayDuplex](contracts/v5.x/api/crosschain#AxelarGatewayDuplex), the library offers an implementation of the [IERC7786GatewaySource](contracts/v5.x/api/crosschain#IERC7786GatewaySource) interface called [AxelarGatewaySource](contracts/v5.x/api/crosschain#AxelarGatewaySource) that works as an adapter for sending messages in compliance with ERC-7786
...
The [AxelarGatewaySource](contracts/v5.x/api/crosschain#AxelarGatewaySource) implementation can be used out of the box

<include cwd lang='solidity'>./examples/crosschain/MyCustomAxelarGatewaySource.sol</include>

For a destination gateway, the library provides an adapter of the `AxelarExecutable` interface to receive messages and relay them to an [IERC7786Receiver](contracts/v5.x/api/crosschain#IERC7786Receiver).

<include cwd lang='solidity'>./examples/crosschain/MyCustomAxelarGatewayDestination.sol</include>

### Open Bridge

The [ERC7786OpenBridge](contracts/v5.x/api/crosschain#ERC7786OpenBridge) is a special gateway that implements both [IERC7786GatewaySource](contracts/v5.x/api/crosschain#IERC7786GatewaySource) and [IERC7786Receiver](contracts/v5.x/api/crosschain#IERC7786Receiver) interfaces. It provides a way to send messages across multiple bridges simultaneously and ensures message delivery through a threshold-based confirmation system.
...
On the receiving end, the bridge implements a threshold-based confirmation system. Messages are only executed after receiving enough confirmations from the gateways, ensuring message validity and preventing double execution. The [`receiveMessage`](contracts/v5.x/api/crosschain#ERC7786OpenBridge-receiveMessage-string-string-string-bytes-bytes---) function handles this process:
...
The bridge is designed to be configurable. As an `Ownable` contract, it allows the owner to manage the list of trusted gateways and adjust the confirmation threshold. The `_gateways` list and threshold are initially set during contract deployment using the [`+_addGateway+`](contracts/v5.x/api/crosschain#ERC7786OpenBridge-_addGateway-address-) and [`+_setThreshold+`](contracts/v5.x/api/crosschain#ERC7786OpenBridge-_setThreshold-uint8-) functions. The owner can update these settings as needed to adapt to changing requirements or add new gateways.
