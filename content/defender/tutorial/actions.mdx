...
<Callout>
Learn to [deploy](:tutorial/deploy) and [monitor](:tutorial/monitor) contracts through Defender!
</Callout>

## 1. Set up action

You will configure an action that sends an hourly transaction with the `addObject()` function to the `0xC64f7ace6127bc7B0bAb23bD1871aC81e6AEC074` contract in Sepolia, which is an example of a Box contract deployed in the [Deploy](:tutorial/deploy) tutorial. To do so, follow these steps:
...
<Callout>
Alongisde actions, we recommend using Access Control to manage a contract’s permissions through Defender. Learn how to use Access Control with its tutorial [here](:tutorial/access-control).
</Callout>

## References

* [Actions Documentation](:module/actions)
* [More information on Action’s Javascript code](:module/actions#defining-code)
