...
Actions allow you to implement custom app logic for on-chain and off-chain operations. You can enable automated responses to threats detected by the [Monitor](module/monitor.mdx) and [Workflows](module/actions.mdx#workflows) modules.

## Use cases

* Automate smart contract operations
* Execute actions as response to a [Monitor](module/monitor.mdx) alert
* Automate [Workflow](module/actions.mdx#workflows) steps with Actions
...
* **Schedule**: Choose a frequency, and Defender will invoke the function at the specified interval. Note that the specified interval is between two consecutive execution starts, _not_ between the end of one run and the beginning of the next one. Alternatively, it’s possible to specify when the action should run using [cron expressions, window=_blank](https://crontab.cronhub.io/).
...
* **Monitor**: Triggered by a Defender [Monitor](module/monitor.mdx). It will contain a body property with the details for the triggering event that you can use to run custom logic.

### Environment

Actions run in a [node 20 environment, window=_blank](https://nodejs.org/dist/latest-v20.x/docs/api/) with 256mb RAM and a 5-minute timeout. The code for each action must be smaller than 5mb in size. For ease-of-use, a set of common dependencies are pre-installed in the environment. The latest action dependenacy version `v2025-01-16` has the following dependencies:
...
<Callout>
If other dependencies are needed, a JavaScript module bundler, such as rollup or webpack, can be used. Refer to [this sample project, window=_blank](https://github.com/OpenZeppelin/defender-sdk/tree/main/examples/custom-ethers-pkg) to learn how. Contact us to add a dependacy you think other users would find useful!
</Callout>

<Callout>
We love [Typescript, window=_blank](https://www.typescriptlang.org/) in the OpenZeppelin development team, and we hope you do too! If you want to write your actions in TypeScript, you’ll need to first compile them using `tsc` or via your bundler of choice, and then upload the resulting JavaScript code. Unfortunately, we don’t support coding directly in TypeScript in the user interface. All `defender-sdk` packages are coded in TypeScript and are packaged with their type declarations. You can also use the [openzeppelin/defender-sdk-action-client, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk-action-client) package for type definitions for the event payload.
</Callout>
...
* In the case of a Defender monitor, the body will contain the [monitor event schema](module/monitor.mdx#monitor_event_schema).

If the action is written in TypeScript, `BlockTriggerEvent` type from the [defender-sdk-action-client, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk-action-client) package can be used.
...
* Download `yarn.lock`: 📎 [yarn.lock](attachmentsdir/yarn.lock)
...
You can edit an action’s code via the Defender interface, or programmatically via API using the [`defender-sdk`, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk) npm package. The latter allows to upload a code bundle with more than a single file:
...
Workflows are processes that combine automatic actions and transaction templates. Actions can be run in parallel or connected sequentially. Workflows can be triggered manually or via a [Monitor](module/monitor.mdx).
...
<Callout>
We provide a quickstart tutorial to create and use Workflows. Check it out [here](tutorial/workflows.mdx)!
</Callout>
...
<Callout>
We provide a quickstart tutorial to create an automatic action for a smart contract using Defender. Check it out [here](tutorial/actions.mdx)!
</Callout>
