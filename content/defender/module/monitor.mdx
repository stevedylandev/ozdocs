...
<Callout type='warn'>
At this time, Monitors are supported across all [networks](index.mdx#networks) except Fantom.
</Callout>
...
The request body will contain the following structure. The `MonitorConditionRequest` type from the [defender-sdk-action-client, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk-action-client) package can be used for custom filters in Typescript.
...
You can create **Notifications** attached to the Monitor alerts for getting notified about the on-chain events across many channels, see more about how to create and configure them in [this section](settings/notifications.mdx).
...
<Callout>
We provide a quickstart tutorial to monitor a smart contract using Defender. Check it out [here](tutorial/monitor.mdx)!
</Callout>
