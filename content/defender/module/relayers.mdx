...
<Callout>
Testnet Relayers created through the Deploy wizard will be automatically funded if possible. Read more [here](:module/deploy.adoc#wizard-3).
</Callout>
...
To configure a Relayer’s policies, go to the [Relayer page, window=_blank](https://defender.openzeppelin.com/v2/#/relayers), select the Relayer, and then go to the **Policies** tab. You will then see a form where you can opt to enable policies and tweak their parameters.
...
The easiest way to send a transaction via a Relayer is using the [`Defender SDK`, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk) package. The client is initialized with an API key/secret and exposes a simple API for sending transactions through the corresponding Relayer.
...
The Relayer client integrates with [ethers.js, window=_blank](https://docs.ethers.io/v6/) via a custom [signer, window=_blank](https://docs.ethers.org/v6/api/providers/#Signer). This allows you switch to a Relayer and send transactions with minimal changes in your codebase.
...
You can read more about the ethers integration [here, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk-relay-client).

### Using web3.js

The Relayer client integrates with [web3.js, window=_blank](https://web3js.readthedocs.io/) as well as via a custom [provider, window=_blank](https://web3js.readthedocs.io/en/v1.3.4/web3-eth.html#providers). This allows you to send transactions with a Relayer and query the network using the familiar web3 interface.
...
You can read more about the web3 integration [here, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk-relay-client).
...
<Callout>
If you notice an EIP1559 compatible network that we already support but doens’t have the EIP enabled, please don’t hesitate to reach out via [https://www.openzeppelin.com/defender2-feedback, window=_blank](https://www.openzeppelin.com/defender2-feedback).
</Callout>
...
* **Mainnet**: Via [Flashbots Protect RPC, window=_blank](https://docs.flashbots.net/flashbots-protect/rpc/quick-start)
...
<Callout>
Mainnet gas prices and priority fees are calculated based on the values reported by [EthGasStation, window=_blank](https://ethgasstation.info/), [EtherChain, window=_blank](https://etherchain.org/tools/gasPriceOracle), [GasNow, window=_blank](https://www.gasnow.org/), [Blockative, window=_blank](https://docs.blocknative.com/gas-platform), and [Etherscan, window=_blank](https://etherscan.io/gastracker). In Polygon and its testnet, the [gas station, window=_blank](https://gasstation-mainnet.matic.network/v2) is used. In other networks, gas prices are obtained from a call to `eth_gasPrice` or `eth_feeHistory` to the network.
</Callout>
...
In addition to sending transactions, a Relayer can also sign arbitrary messages according to the [EIP-191 Standard, window=_blank](https://eips.ethereum.org/EIPS/eip-191) (prefixed by `\x19Ethereum Signed Message:\n`) using its private key. You can access this feature via the `sign` method of the client or the equivalent ethers.js method.
...
Along with the sign api method, Relayers also implement a `signTypedData`, which you can use to sign messages according to the [EIP712 Standard, window=_blank](https://eips.ethereum.org/EIPS/eip-712) for typed data signatures.
...
If you are using ethers.js, this is supported via a custom `DefenderRelayProvider` [provider, window=_blank](https://docs.ethers.org/v6/api/providers/) object:
...
You can withdraw funds from a Relayer on the [Relayers page, window=_blank](https://defender.openzeppelin.com/v2/#/relayers), selecting the Relayer, and clicking on **Withdraw**.
...
You may use the [`Defender SDK`, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk) package to load balance across multiple relayers. Here is a simple example on how you can do this:
