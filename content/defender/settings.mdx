...
They are defined per network, and are used throughout Defender to execute transactions, such as in the [Deploy](module/deploy) wizard, [Actions](module/actions) and [Workflows](module/actions#workflows).
...
For more information on the event schema see the documentation on [Monitor](module/monitor) or [Actions](module/actions).
...
<Callout>
The chain ID of a private network must not conflict with the chain ID of an [officially supported Defender network](index#networks).
</Callout>
