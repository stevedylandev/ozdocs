---
title: Relaying gasless meta-transactions with a web app
description: Relaying gasless meta-transactions with a web app
---
...
A gasless meta-transaction relay can be easily and securely implemented using Defender with [Relayers](:module/relayers.adoc), which allow you to send transactions easily without needing to manage private keys, transaction signing, nonce management, gas estimation, and transaction inclusion.

## Pre-requisites

* OpenZeppelin Defender account.
* [Git, window=_blank](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git) and [Yarn, window=_blank](https://classic.yarnpkg.com/lang/en/docs/install/#mac-stable) installed

## 1. ERC-2771: Secure Native Meta Transactions

You can view the live [demo app, window=_blank](https://defender-metatx-workshop-demo.openzeppelin.com/) here. It accepts registrations directly if the user has the available funds to pay for the transaction, otherwise the data is sent as a meta-transaction.

In the example code, the functionality of the [`SimpleRegistry` contract, window=_blank](https://github.com/OpenZeppelin/workshops/blob/master/25-defender-metatx-api/contracts/SimpleRegistry.sol) is to take a string and store it. The contract’s [meta-transaction implementation, window=_blank](https://github.com/OpenZeppelin/workshops/blob/master/25-defender-metatx-api/contracts/Registry.sol) achieves the same result by decoupling the signer from the sender of the transaction.
...
The second fundamental change between the two contracts is the need for the meta-transaction contract ([Registry, window=_blank](https://github.com/OpenZeppelin/workshops/blob/master/25-defender-metatx-api/contracts/Registry.sol)) to specify the address of the trusted forwarder, which in this case is the address of the `ERC2771Forwarder` contract.
...
Create a `.env` file in the project root and supply an API key and secret from the [API keys page, window=_blank](https://defender.openzeppelin.com/v2/#/settings/api-keys/new). A private key will be used for local testing but the Relayer is used for actual contract deployment.
...
You can easily deploy a compiled smart contract without handling a private key by using the Relayer client from the [`defender-sdk`, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk) package.
...
The demo app uses an [Action](module/actions.mdx) to supply the necessary logic for telling the Relayer to send a transaction to the `Forwarder` contract, supplying the signer’s address. The Action will get triggered by each call to its webhook from the app.
...
Note that the Action code must include an `index.js` file that exports a handler entrypoint. If the code relies on any external dependencies (such as an imported ABI) it’s necessary to bundle the Action using webpack, rollup, etc. You can create an Action via [Defender, window=_blank](https://defender.openzeppelin.com/v2/#/actions/automatic/new?) or with the [`defender-sdk`, window=_blank](https://www.npmjs.com/package/@openzeppelin/defender-sdk) package.
...
Head to [Defender Actions, window=_blank](https://defender.openzeppelin.com/v2/#/actions/automatic) and copy the Actions’s webhook so that you can test functionality and connect the app to the Action for relaying meta-transactions.

![Copy Webhook](/defender/guide-meta-tx-copy-webhook.png)
...
1. Open app: [http://localhost:3000/, window=_blank](http://localhost:3000/)
...
* [Demo repo - Meta-Transaction Name Registry, window=_blank](https://github.com/OpenZeppelin/workshops/tree/master/25-defender-metatx-api)
* [Documentation - Meta Transactions, window=_blank](https://docs.openzeppelin.com/contracts/api/metatx)
