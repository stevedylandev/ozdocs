...
1. ***[`Upgradeable`](#upgrade_only)*** for cases where only the WASM binary needs to be updated.
2. ***[`UpgradeableMigratable`](#upgrade_and_migrate)*** for more advanced scenarios where, in addition to the WASM binary, specific storage entries
