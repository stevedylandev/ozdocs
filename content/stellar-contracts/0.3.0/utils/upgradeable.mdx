...
1. ***[`Upgradeable`](utils/upgradeable.mdx#upgrade_only)*** for cases where only the WASM binary needs to be updated.
2. ***[`UpgradeableMigratable`](utils/upgradeable.mdx#upgrade_and_migrate)*** for more advanced scenarios where, in addition to the WASM binary, specific storage entries
