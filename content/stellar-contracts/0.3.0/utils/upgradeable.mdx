...
1. ***[`Upgradeable`](/stellar-contracts/0.3.0/utils/upgradeable#upgrade-only)*** for cases where only the WASM binary needs to be updated.
2. ***[`UpgradeableMigratable`](/stellar-contracts/0.3.0/utils/upgradeable#upgrade-and-migrate)*** for more advanced scenarios where, in addition to the WASM binary, specific storage entries
