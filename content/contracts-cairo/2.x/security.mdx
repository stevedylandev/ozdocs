...
The [Initializable](/contracts-cairo/2.x/api/security#InitializableComponent) component provides a simple mechanism that mimics
...
The [Pausable](/contracts-cairo/2.x/api/security#PausableComponent) component allows contracts to implement an emergency stop mechanism.
...
For example (using the [Ownable](/contracts-cairo/2.x/api/access#OwnableComponent) component for access control):
...
Since Cairo does not support modifiers like Solidity, the [ReentrancyGuard](/contracts-cairo/2.x/api/security#ReentrancyGuardComponent)
