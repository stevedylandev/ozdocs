...
4. `__validate_deploy__` optional entrypoint similar to `__validate__` but meant for [counterfactual deployments](/contracts-cairo/guides/deployment).
...
The former can be easily done with the [Universal Deployer Contract (UDC)](/contracts-cairo/1.0.0/udc), a contract that
...
`__validate_deploy__`. Check the [counterfactual deployments](/contracts-cairo/guides/deployment) guide to learn how.
