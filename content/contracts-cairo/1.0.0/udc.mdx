...
Achieving this type of deployment necessitates that the origin sets `from_zero` to `false` in the [deploy_contract](/contracts-cairo/api/udc#UniversalDeployer-deploy_contract) call.
...
<Callout>
See the [previous Universal Deployer API](https://docs.starknet.io/architecture-and-concepts/accounts/universal-deployer/) for the initial spec.
</Callout>

The latest iteration of the UDC includes some notable changes to the API which include:

* `deployContract` method is replaced with the snake_case [deploy_contract](/contracts-cairo/api/udc#UniversalDeployer-deploy_contract).
* [Pedersen](https://docs.starknet.io/architecture-and-concepts/cryptography/#pedersen_hash) hashing algorithm is replaced with the [Poseidon](https://docs.starknet.io/architecture-and-concepts/cryptography/#poseidon_hash) hashing algorithm for computing the salt on [origin-dependent deployments](#origin_dependent).
* `unique` parameter is replaced with `from_zero` in both the `deploy_contract` method and [ContractDeployed](/contracts-cairo/api/udc#IUniversalDeployer-ContractDeployed) event.
...
They include the generic [calculate_contract_address_from_deploy_syscall](/contracts-cairo/api/utilities#deployments-calculate_contract_address_from_deploy_syscall) as well as the UDC-specific [calculate_contract_address_from_udc](/contracts-cairo/api/utilities#deployments-calculate_contract_address_from_udc).
Check out the [deployments](/contracts-cairo/api/utilities#deployments) for more information.
