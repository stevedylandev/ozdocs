...
The `path` argument should be the imported component itself (in this case, [InitializableComponent](/security.mdx#initializable)).
...
Components also separate granular implementations from [mixin](/components.mdx#mixins) implementations.
The API documentation design reflects these groupings.
See [ERC20Component](/api/erc20.mdx#ERC20Component) as an example which includes:
...
Let’s look at how a contract would integrate [OwnableComponent](/api/access.mdx#OwnableComponent):
...
Here’s an example of how to use the Immutable Component Config pattern with the [ERC2981Component](/api/token_common.mdx#ERC2981Component):
...
For instance, [AccessControlComponent](/api/access.mdx#AccessControlComponent) depends on [SRC5Component](/api/introspection.mdx#SRC5Component).
...
Next, the contract includes the [ERC20Component](/api/erc20.mdx#ERC20Component) implementations; however, `ERC20Impl` and `ERC20CamelOnlyImplt` are **not** embedded.
...
<Callout>
The ERC20 documentation provides another custom implementation guide for [Customizing decimals](/erc20.mdx#customizing_decimals).
</Callout>
