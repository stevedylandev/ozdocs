...
The [VotesComponent](api/governance.mdx#VotesComponent) provides a flexible system for tracking and delegating voting power. This system allows users to delegate their voting power to other addresses, enabling more active participation in governance.
...
<Callout type='warn'>
The transferring of voting units must be handled by the implementing contract. In the case of `ERC20` and `ERC721` this is usually done via the hooks. You can check the [usage](../governance.mdx#usage_2) section for examples of how to implement this.
</Callout>

## Key Features

1. **Delegation**: Users can delegate their voting power to any address, including themselves. Vote power can be delegated either by calling the [delegate](api/governance.mdx#VotesComponent-delegate) function directly, or by providing a signature to be used with [delegate_by_sig](api/governance.mdx#VotesComponent-delegate_by_sig).
...
When integrating the `VotesComponent`, the [VotingUnitsTrait](api/governance.mdx#VotingUnitsTrait) must be implemented to get the voting units for a given account as a function of the implementing contract.\
...
Additionally, you must implement the [NoncesComponent](api/utilities.mdx#NoncesComponent) and the [SNIP12Metadata](api/utilities.mdx#snip12) trait to enable delegation by signatures.
