...
The [VotesComponent](/contracts-cairo/api/governance#VotesComponent) provides a flexible system for tracking and delegating voting power. This system allows users to delegate their voting power to other addresses, enabling more active participation in governance.
...
1. **Delegation**: Users can delegate their voting power to any address, including themselves. Vote power can be delegated either by calling the [delegate](/contracts-cairo/api/governance#VotesComponent-delegate) function directly, or by providing a signature to be used with [delegate_by_sig](/contracts-cairo/api/governance#VotesComponent-delegate_by_sig).
...
When integrating the `VotesComponent`, the [VotingUnitsTrait](/contracts-cairo/api/governance#VotingUnitsTrait) must be implemented to get the voting units for a given account as a function of the implementing contract.\
...
Additionally, you must implement the [NoncesComponent](/contracts-cairo/api/utilities#NoncesComponent) and the [SNIP12Metadata](/contracts-cairo/api/utilities#snip12) trait to enable delegation by signatures.
