...
OpenZeppelin Contracts for Cairo provides [OwnableComponent](api/access.mdx#OwnableComponent) for implementing ownership in your contracts.
...
[`initializer`](/api/access.mdx#OwnableComponent-initializer) like this:
...
[OwnableTwoStepImpl](/api/access.mdx#OwnableTwoStepImpl) implementation. A two step transfer mechanism helps
...
instead of simply using [`assert_only_owner`](/api/access.mdx#OwnableComponent-assert_only_owner). This check can be enforced through [`assert_only_role`](/api/access.mdx#AccessControlComponent-assert_only_role).
...
Here’s a simple example of implementing [AccessControl](api/access.mdx#AccessControlComponent) on a portion of an ERC20 token contract which defines
...
<Callout type='warn'>
Make sure you fully understand how [AccessControl](api/access.mdx#AccessControlComponent) works before
</Callout>
...
[Ownable](api/access.mdx#OwnableComponent). Where [AccessControl](api/access.mdx#AccessControlComponent) shines the most is in scenarios where granular
...
The ERC20 token example above uses [`_grant_role`](api/access.mdx#AccessControlComponent-_grant_role),
...
the [`assert_only_role`](api/access.mdx#AccessControlComponent-assert_only_role) check pass. To grant and revoke roles dynamically, you will need help from the role’s _admin_.

Every role has an associated admin role, which grants permission to call the
[`grant_role`](api/access.mdx#AccessControlComponent-grant_role) and
[`revoke_role`](api/access.mdx#AccessControlComponent-revoke_role) functions.
...
[`set_role_admin`](api/access.mdx#AccessControlComponent-set_role_admin) is used to select a new admin role.

Since it is the admin for all roles by default, and in fact it is also its own admin, this role carries significant risk. To mitigate this risk we provide [AccessControlDefaultAdminRules](api/access.mdx#AccessControlDefaultAdminRulesComponent), a recommended extension of AccessControl that adds a number of enforced security measures for this role: the admin is restricted to a single account, with a 2-step transfer procedure with a delay in between steps.
