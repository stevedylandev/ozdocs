...
The [VestingComponent](/contracts-cairo/api/finance#VestingComponent) manages the gradual release of ERC-20 tokens to a designated beneficiary based on a predefined vesting schedule.
The implementing contract must implement the [OwnableComponent](/contracts-cairo/api/access#OwnableComponent), where the contract owner is regarded as the vesting beneficiary.
...
The [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) trait defines the logic for calculating the vested amount based on a given timestamp. This
logic is not part of the [VestingComponent](/contracts-cairo/api/finance#VestingComponent), so any contract implementing the [VestingComponent](/contracts-cairo/api/finance#VestingComponent) must provide its own
implementation of the [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) trait.

<Callout>
There’s a ready-made implementation of the [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) trait available named [LinearVestingSchedule](/contracts-cairo/api/finance#LinearVestingSchedule).
...
The contract must integrate [VestingComponent](/contracts-cairo/api/finance#VestingComponent) and [OwnableComponent](/contracts-cairo/api/access#OwnableComponent) as dependencies. The contract’s constructor
...
The implementing contract must provide an implementation of the [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) trait. This can be achieved either by importing
a ready-made [LinearVestingSchedule](/contracts-cairo/api/finance#LinearVestingSchedule) implementation or by defining a custom one.

Here’s an example of a simple vesting wallet contract with a [LinearVestingSchedule](/contracts-cairo/api/finance#LinearVestingSchedule), where the vested amount
...
A vesting schedule will often follow a custom formula. In such cases, the [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) trait is useful.
To support a custom vesting schedule, the contract must provide an implementation of the
[calculate_vested_amount](/contracts-cairo/api/finance#VestingComponent-calculate_vested_amount) function based on the desired formula.

<Callout>
When using a custom [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) implementation, the [LinearVestingSchedule](/contracts-cairo/api/finance#LinearVestingSchedule) must be excluded from the imports.
</Callout>
...
Here’s an example of a vesting wallet contract with a custom [VestingSchedule](/contracts-cairo/api/finance#VestingComponent-Vesting-Schedule) implementation, where tokens
