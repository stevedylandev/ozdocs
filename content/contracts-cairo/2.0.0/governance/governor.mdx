...
to implement the [VotesComponent](/contracts-cairo/api/governance#VotesComponent) extension. This extension will keep track of historical balances so that voting power
...
We will initially build a Governor without a timelock. The core logic is given by the [GovernorComponent](/contracts-cairo/api/governance#GovernorComponent), but we
...
***For 1)*** we will use the GovernorVotes extension, which hooks to an [IVotes](/contracts-cairo/api/governance#IVotes) instance to determine the voting power
...
***For 2)*** we will use GovernorVotesQuorumFraction. This works together with the [IVotes](/contracts-cairo/api/governance#IVotes) instance to define the quorum as a
...
with a decision before it is executed. We will use OpenZeppelin’s [TimelockController](/contracts-cairo/2.0.0/governance/timelock) in combination with the
