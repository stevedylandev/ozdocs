...
The [Initializable](/contracts-cairo/api/security#InitializableComponent) component provides a simple mechanism that mimics
...
The [Pausable](/contracts-cairo/api/security#PausableComponent) component allows contracts to implement an emergency stop mechanism.
...
For methods that should be available only when paused or not, insert calls to `[assert_paused](/contracts-cairo/api/security#PausableComponent-assert_paused)` and `[assert_not_paused](/contracts-cairo/api/security#PausableComponent-assert_not_paused)`
respectively.

### Usage

For example (using the [Ownable](/contracts-cairo/api/access#OwnableComponent) component for access control):
...
Since Cairo does not support modifiers like Solidity, the [ReentrancyGuard](/contracts-cairo/api/security#ReentrancyGuardComponent)
component exposes two methods `[start](/contracts-cairo/api/security#ReentrancyGuardComponent-start)` and `[end](/contracts-cairo/api/security#ReentrancyGuardComponent-end)` to protect functions against reentrancy attacks.
