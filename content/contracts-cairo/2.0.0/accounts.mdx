...
<Callout>
For detailed information on the usage and implementation check the [API Reference](/contracts-cairo/api/account) section.
</Callout>
...
4. `__validate_deploy__` optional entrypoint similar to `__validate__` but meant for [counterfactual deployments](/contracts-cairo/2.0.0/guides/deployment).
...
OpenZeppelin Contracts for Cairo provides [AccountComponent](/contracts-cairo/api/account#AccountComponent) for implementing this validation scheme.

### Usage

Constructing an account contract requires integrating both [AccountComponent](/contracts-cairo/api/account#AccountComponent) and [SRC5Component](/contracts-cairo/api/introspection#SRC5Component). The contract should also set up the constructor to initialize the public key that will be used as the account’s signer. Here’s an example of a basic contract:
...
For this the [EthAccountComponent](/contracts-cairo/api/account#EthAccountComponent) must be used.

### Usage

Constructing a secp256k1 account contract also requires integrating both [EthAccountComponent](/contracts-cairo/api/account#EthAccountComponent) and [SRC5Component](/contracts-cairo/api/introspection#SRC5Component).
...
The former can be easily done with the [Universal Deployer Contract (UDC)](/contracts-cairo/2.0.0/udc), a contract that
...
`__validate_deploy__`. Check the [counterfactual deployments](/contracts-cairo/2.0.0/guides/deployment) guide to learn how.
