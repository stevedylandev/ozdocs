...
The contract should include the new introspection mechanism, [SRC5Component](/contracts-cairo/api/introspection#SRC5Component).
It should also include the [InitializableComponent](/contracts-cairo/api/security#InitializableComponent) which will be used in the `migrate` function.
Here’s the setup:
...
For this example, let’s say that this contract supports the [IERC721](/contracts-cairo/api/erc721#IERC721) and [IERC721Metadata](/contracts-cairo/api/erc721#IERC721Metadata) interfaces.
...
Since the new contract integrates `SRC5Component`, it can leverage SRC5’s [register_interface](/contracts-cairo/api/introspection#SRC5Component-register_interface) function to register the supported interfaces.
...
Since the `migrate` function will be publicly callable, it should include some sort of [Access Control](/contracts-cairo/2.0.0/access) so that only permitted addresses can execute the migration.
