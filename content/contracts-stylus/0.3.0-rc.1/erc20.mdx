...
An ERC-20 token contract keeps track of [_fungible_ tokens](tokens.mdx#different-kinds-of-tokens): any token is exactly equal to any other token; no token has a special right or behavior associated with them.
...
On the [`API reference`](https://docs.rs/openzeppelin-stylus/0.3.0-alpha.1/openzeppelin_stylus/token/erc20/struct.Erc20.html) you’ll find detailed information on their properties and usage.
...
To work around this, ERC20 provides a [`decimals`](https://docs.rs/openzeppelin-stylus/0.3.0-alpha.1/openzeppelin_stylus/token/erc20/extensions/metadata/trait.IErc20Metadata.html#tymethod.decimals) field, which is used to specify how many decimal places a token has.
...
* [ERC-20 Burnable](erc20-burnable.mdx): destruction of own tokens.
* [ERC-20 Capped](erc20-capped.mdx): enforcement of a cap to the total supply when minting tokens.
* [ERC-20 Metadata](erc20-metadata.mdx): the extended ERC20 interface including the name, symbol, and decimals functions.
* [ERC-20 Pausable](erc20-pausable.mdx): ability to pause token transfers.
* [ERC-20 Permit](erc20-permit.mdx): gasless approval of tokens (standardized as [`EIP-2612`](https://eips.ethereum.org/EIPS/eip-2612)).
* [ERC-4626](erc4626.mdx): tokenized vault that manages shares (represented as ERC-20) that are backed by assets (another ERC-20).
* [ERC-20 Flash-Mint](erc20-flash-mint.mdx): token level support for flash loans through the minting and burning of ephemeral tokens (standardized as [`EIP-3156`](https://eips.ethereum.org/EIPS/eip-3156)).
* [ERC-20 Wrapper](erc20-wrapper.mdx): wrapper to create an ERC-20 backed by another ERC-20, with deposit and withdraw methods.
