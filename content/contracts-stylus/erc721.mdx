...
We’ve discussed how you can make a _fungible_ token using [ERC-20](erc20.mdx), but what if not all tokens are alike?
...
ERC-721 is a standard for representing ownership of [_non-fungible_ tokens](tokens.mdx#different-kinds-of-tokens), that is, where each token is unique.
...
Check out the [`API reference`](https://docs.rs/openzeppelin-stylus/0.3.0-alpha.1/openzeppelin_stylus/token/erc721/struct.Erc721.html) to learn more about these.
...
We can use an [Access Control](access-control.mdx) extension.
...
The [`Erc721Metadata`](https://docs.rs/openzeppelin-stylus/0.3.0-alpha.1/openzeppelin_stylus/token/erc721/extensions/metadata/struct.Erc721Metadata.html) contract is an extension contract of ERC-721.
...
* [ERC-721 Burnable](erc721-burnable.mdx): A way for token holders to burn their own tokens.
* [ERC-721 Consecutive](erc721-consecutive.mdx): An implementation of [ERC2309](https://eips.ethereum.org/EIPS/eip-2309) for minting batches of tokens during construction, in accordance with ERC721.
* [ERC-721 Enumerable](erc721-enumerable.mdx): Optional extension that allows enumerating the tokens on chain, often not included since it requires large gas overhead.
* [ERC-721 Metadata](erc721-metadata.mdx): Optional extension that adds name, symbol, and token URI, almost always included.
* [ERC-721 Pausable](erc721-pausable.mdx): A primitive to pause contract operation.
* [ERC-721 Uri Storage](erc721-uri-storage.mdx): A more flexible but more expensive way of storing metadata.
* [ERC-721 Wrapper](erc721-wrapper.mdx): Wrapper to create an ERC-721 backed by another ERC-721, with deposit and withdraw methods.
