...
>
  ...
  * Caller must be `from` or approved via [setApprovalForAll](#MulitToken-setapprovalforall).
...
<APIItemCompact
  ...
  modulePrefix="MultiToken"
>
  Unsafe variant of [transferFrom](#MulitToken-transferfrom) which allows transfers to contract addresses.
  The caller must be `from` or approved to transfer on their behalf.
...
<APIItemCompact
  ...
  modulePrefix="MultiToken"
>
  Unsafe variant of [_transfer](#MulitToken-_transfer) which allows transfers to contract addresses.
