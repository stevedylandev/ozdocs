---
title: FungibleToken
description: FungibleToken
---

FungibleToken is a specification for [fungible tokens](https://docs.openzeppelin.com/contracts/5.x/tokens#different-kinds-of-tokens),
a type of token where all the units are exactly equal to each other.
This module is an approximation of [EIP-20](https://eips.ethereum.org/EIPS/eip-20) written in the Compact programming language for the Midnight network.
...
Exposing [_mint](/api/fungibleToken.mdx#FungibleTokenModule-_mint) without some sort of access control, for example, would allow ANYONE to mint tokens.
