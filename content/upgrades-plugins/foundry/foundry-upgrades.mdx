...
If you do not want to run upgrade safety validations, you can skip the above steps and use the [`unsafeSkipAllChecks` option](api#Options) when calling the `Upgrades` library’s functions, or use the `UnsafeUpgrades` library instead. Note that these are dangerous options meant to be used as a last resort.
...
If you are using OpenZeppelin Defender, see [OpenZeppelin Defender with Foundry](foundry-defender) for how to use it for deployments.

## API

See [Foundry Upgrades API](api) for the full API documentation.
