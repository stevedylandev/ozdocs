...
See [Using with Foundry - Installation](/upgrades-plugins/foundry/foundry-upgrades#installation).
...
If you are deploying upgradeable contracts, use the `Upgrades` library as described in [Using with Foundry - Installation](/upgrades-plugins/foundry/foundry-upgrades#installation) but set the option `defender.useDefenderDeploy = true` when calling functions to cause all deployments to occur through OpenZeppelin Defender.
