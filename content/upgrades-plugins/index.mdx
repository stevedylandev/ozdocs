...
<Callout>
Upgrades Plugins are only a part of a comprehensive set of OpenZeppelin tools for deploying and securing upgradeable smart contracts. [Check out the full list of resources](ROOT::upgrades).
</Callout>
...
See the documentation for [Hardhat Upgrades](hardhat-upgrades) or [Foundry Upgrades](foundry-upgrades).
...
1. Validates that the implementation is [upgrade safe](faq#what-does-it-mean-for-a-contract-to-be-upgrade-safe).
2. Deploys the [implementation contract](faq#what-is-an-implementation-contract). Note that the Hardhat plugin first checks if there is an implementation contract deployed with the same bytecode, and skips this step if one is already deployed.
3. Creates and initializes the proxy contract, along with a [proxy admin](faq#what-is-a-proxy-admin) (if needed).

And when you call `upgradeProxy`:

1. Validates that the new implementation is [upgrade safe](faq#what-does-it-mean-for-a-contract-to-be-upgrade-safe) and is [compatible](faq#what-does-it-mean-for-an-implementation-to-be-compatible) with the previous one.
2. Deploys the new [implementation contract](faq#what-is-an-implementation-contract). Note that the Hardhat plugin first checks if there is an implementation contract deployed with the same bytecode, and skips this step if one is already deployed.
...
For UUPS and transparent proxies, use `deployProxy` and `upgradeProxy`. For beacon proxies, use `deployBeacon`, `deployBeaconProxy`, and `upgradeBeacon`. See the documentation for [Hardhat Upgrades](hardhat-upgrades) and [Foundry Upgrades](foundry-upgrades) for examples.

## Managing ownership

Transparent proxies define an _admin_ address which has the rights to upgrade them. By default, the admin is a [proxy admin contract](faq#what-is-a-proxy-admin) deployed behind the scenes. Keep in mind that the _admin_ of a proxy can only upgrade it, but not interact with the implementation contract. Read [Transparent Proxies and Function Clashes](proxies#transparent-proxies-and-function-clashes) for more info on this restriction.
