...
See [Storage Configuration](storage.mdx) for detailed information about how file-based and API-based configurations work together, storage behavior, and best practices.
...
| `REPOSITORY_STORAGE_TYPE` | `in-memory` | `in-memory, redis` | Type of storage used for storing repository config and resources. See [Storage Configuration](storage.mdx) for detailed information. |
| `RESET_STORAGE_ON_START` | `false` | `bool` | Clears all resources from storage on startup and reloads entries from the config file. See [Storage Configuration](storage.mdx) for usage details. |
| `TRANSACTION_EXPIRATION_HOURS` | `4` | `number` | Number of hours after which transactions in a final state are removed from storage. See [Storage Configuration](storage.mdx) for more information. |
...
| `REDIS_URL` | `redis://localhost:6379` | `<redis connection string>` | Redis connection URL for the relayer. See [Storage Configuration](storage.mdx) for Redis setup details. |
| `REDIS_CONNECTION_TIMEOUT_MS` | `10000` | `<timeout in milliseconds>` | Connection timeout for Redis in milliseconds. See [Storage Configuration](storage.mdx) for Redis configuration. |
| `REDIS_KEY_PREFIX` | `oz-relayer` | `string` | Redis key prefix for namespacing. See [Storage Configuration](storage.mdx) for more information. |
| `STORAGE_ENCRYPTION_KEY` | `` | `string` | Encryption key used to encrypt data at rest in Redis storage. See [Storage Configuration](storage.mdx) for security details. |
...
See the dedicated [Signers Configuration](signers.mdx) guide.
...
| network | String | Network the relayer will connect to. Must match a network identifier defined in your network configuration files. See [Network Configuration](network_configuration.mdx) for details on defining networks. |
...
| policies | list | Overrides default policies. Please refer to the [`Policies`](configuration.mdx#network_policies) table |
...
For more information on how to write a plugin, please refer to the [Plugins](plugins.mdx) page.
...
See the dedicated [Network Configuration](network_configuration.mdx) guide.
