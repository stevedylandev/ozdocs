...
* Move to the directory of the template you want to use. We will use the `generic runtime template` for this tutorial,but the same applies to the [EVM Runtime Template](runtimes/evm.mdx).
```bash
cd generic-template
```
...
* Learn more about the runtime configuration. Currently, we have two runtime templates: [Generic Runtime Template](runtimes/generic.mdx) and [EVM Runtime Template](runtimes/evm.mdx).
