...
In substrate, every transaction has an associated `call`, and each `call` has its own [weight](glossary#weight) function. The weight function estimates the time it takes to execute the `call`.
...
The base fee and adjusted [weight](glossary#weight_fee) and [length](glossary#length_fee) fees constitute the _inclusion fee_, which is the minimum fee for a transaction to be included in a block. The formula of final fee:
```rust, ignore
