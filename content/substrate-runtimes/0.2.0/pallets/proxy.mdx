...
* Pallet-specific configs:
  * `ProxyType` -- a type that describes different variants of [proxy](glossary#proxy). It must implement `Default` trait and `InstanceFilter<RuntimeCall>` trait.
  ...
  * `MaxPending` -- maximum number of [announcements](glossary#announcement) that can be made per account.
...
This call will take (or modify) a deposit based on number of proxies created by [delegator](glossary#delegator) and calculated by this formula: `ProxyDepositBase + ProxyDepositFactor * <number of proxies>`
...
* `TooMany` — too many proxies created for this delegate with this type and the same [delay](glossary#delay)
...
* `real: AccountIdLookupOf<T>` — the account on which behalf this call will be made
* `force_proxy_type: Option<T::ProxyType>` — specific [proxy type](glossary#proxy_type) to get proxy for. If not specified, first one found in the storage will be used.
...
May be called from delegator of the proxy to remove announcement made by [delegatee](glossary#delegatee).
...
This call creates a [pure account](glossary#pur_account) with a proxy issued to it from the call’s origin.
