...
## Source Code: link:https://github.com/paritytech/polkadot-sdk/blob/release-polkadot-v1.10.0/substrate/frame/multisig/src/lib.rs[pass:[<svg class="icon"><use href="#github-icon"/></svg>],role=heading-link]
...
* Pallet-specific configs
  * `DepositBase` -- The base amount of currency needed to [reserve](/substrate-runtimes/2.0.2/glossary#reserve) for creating a multisig execution or to store a dispatch call for later. Recall: The deposit to be made by the account that creates the multisig is: `threshold * DepositFactor + DepositBase`
...
* Common configs
  ...
  * `WeightInfo` -- [Weight](/substrate-runtimes/2.0.2/glossary#weight) information for extrinsics in this pallet.
...
* funding the multisig account (same for `1-out-of-n multisig accounts`, and `m-out-of-n multisig accounts`)
  ...
  * 👉 check this documentation if you have questions about accounts and their creation [multisig_accounts](/substrate-runtimes/2.0.2/misc/multisig-accounts)
