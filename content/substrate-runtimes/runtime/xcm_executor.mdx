...
* Handlers:
  ...
  * `Trader` --  The means of purchasing [weight](glossary.mdx#weight) credit for XCM execution. Type must implement the trait `WeightTrader`.
...
* Filters:
  ...
  * `UniversalAliases` -- The origin locations and specific universal [junctions](glossary.mdx#junctions) to which they are allowed to elevate themselves. Type must implement the trait `Contains<(MultiLocation, Junction)>`.
