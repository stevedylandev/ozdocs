...
* Handlers:
  ...
  * `Trader` --  The means of purchasing [weight](glossary#weight) credit for XCM execution. Type must implement the trait `WeightTrader`.
...
* Filters:
  ...
  * `UniversalAliases` -- The origin locations and specific universal [junctions](glossary#junctions) to which they are allowed to elevate themselves. Type must implement the trait `Contains<(MultiLocation, Junction)>`.
