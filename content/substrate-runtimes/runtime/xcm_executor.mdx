...
[Source Code](https://github.com/paritytech/polkadot-sdk/blob/release-polkadot-v1.10.0/polkadot/xcm/xcm-executor/src/config.rs)
...
* Handlers:
  ...
  * `Trader` --  The means of purchasing [weight](/substrate-runtimes/glossary#weight) credit for XCM execution. Type must implement the trait `WeightTrader`.
...
* Filters:
  ...
  * `UniversalAliases` -- The origin locations and specific universal [junctions](/substrate-runtimes/glossary#junctions) to which they are allowed to elevate themselves. Type must implement the trait `Contains<(MultiLocation, Junction)>`.
...
* `error_handler` -- Type must be `Xcm<Config::RuntimeCall>`.
