...
* Pallet-specific origins:
  ...
  * `ExecuteXcmOrigin` -- Required origin for executing XCM messages, including the teleport functionality. If successful, it returns a `[MultiLocation](glossary.mdx#multilocation)` which exists as an interior location within this chain’s XCM context. Type must implement trait `EnsureOrigin`.
...
* Pallet-specific handlers:
  ...
  * `Weigher` -- Measures [weight](glossary.mdx#weight) consumed by XCM local execution. Type must implement the trait `WeightBounds`.
...
* Pallet-specific converters:
  * `CurrencyMatcher` -- The `[MultiAsset](glossary.mdx#multiasset)` matcher for `Currency`. Type must implement the trait `MatchesFungible`.
