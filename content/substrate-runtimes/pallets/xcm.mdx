...
Source Code: [GitHub](https://github.com/paritytech/polkadot-sdk/blob/release-polkadot-v1.10.0/polkadot/xcm/pallet-xcm/src/lib.rs)
...
* Pallet-specific origins:
  ...
  * `ExecuteXcmOrigin` -- Required origin for executing XCM messages, including the teleport functionality. If successful, it returns a `[MultiLocation](/substrate-runtimes/glossary#multilocation)` which exists as an interior location within this chain’s XCM context. Type must implement trait `EnsureOrigin`.
...
* Pallet-specific handlers:
  ...
  * `Weigher` -- Measures [weight](/substrate-runtimes/glossary#weight) consumed by XCM local execution. Type must implement the trait `WeightBounds`.
...
* Pallet-specific converters:
  * `CurrencyMatcher` -- The `[MultiAsset](/substrate-runtimes/glossary#multiasset)` matcher for `Currency`. Type must implement the trait `MatchesFungible`.
...
* `SupportedVersionChanged(location, version)` -- `location` was updated to support the latest version of XCM `version`
...
* `Attempted(outcome)` -- Attempted the reserve transfer with returned status `outcome`
...
* `Attempted(outcome)` -- Attempted the teleport with returned status `outcome`
...
* `assets: Box<VersionedAssets>` -- The assets to be withdrawn. This should include the assets used to pay the fee on the `dest` (and possibly reserve) chains.
* `fee_asset_item: u32` -- The index into `assets` of the item which should be used to pay fees.
* `weight_limit: WeightLimit` -- The remote-side weight limit, if any, for the XCM fee purchase.
...
[Polkadot Wiki XCM Use Cases](https://wiki.polkadot.network/docs/learn-xcm-usecases)
