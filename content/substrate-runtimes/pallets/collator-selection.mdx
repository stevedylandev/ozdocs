...
This pallet is needed to manage the set of [collators](glossary#collator) for each session and to provision the next session with the actual list of collators.

## Config

* Pallet-specific configs
  * `UpdateOrigin` — defines the list of origins that are able to modify the settings of collators (e.g. set and remove list of [invulnerables](glossary#invulnerable), desired [candidates](glossary#candidates), [candidacy bond](glossary#candidacy_bond)). This type should implement the trait `EnsureOrigin`.
  * `PotId` — id of account that will hold a [Pot](glossary#pot).
