...
* Pallet-specific configs
  * `DepositBase` -- The base amount of currency needed to [reserve](glossary#reserve) for creating a multisig execution or to store a dispatch call for later. Recall: The deposit to be made by the account that creates the multisig is: `threshold * DepositFactor + DepositBase`
...
* funding the multisig account (same for `1-out-of-n multisig accounts`, and `m-out-of-n multisig accounts`)
  ...
  * 👉 check this documentation if you have questions about accounts and their creation [multisig_accounts](misc/multisig-accounts)
