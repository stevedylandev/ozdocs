...
description: NonFungibleToken
---

NonFungibleToken is a specification for [non-fungible tokens](https://docs.openzeppelin.com/contracts/5.x/tokens#different-kinds-of-tokens),
a type of token where all the units are unique and distinct from each other.
This module is an approximation of [EIP-721](https://eips.ethereum.org/EIPS/eip-721) written in the Compact programming language for the Midnight network.
...
it’s not possible to implement an [ERC-165](https://eips.ethereum.org/EIPS/eip-165) like interface standard at this time.
* ***Safe Transfers*** - It’s not possible to implement safe transfers without an [ERC-165](https://eips.ethereum.org/EIPS/eip-165) like
...
This library enables extensibility by following the rules of the [Module/Contract Pattern](extensibility.mdx#the_module_contract_pattern).
Note that circuits with a preceding underscore (`_likeThis`) are meant to be building blocks for implementing contracts.
Exposing [_mint](/api/nonFungibleToken.mdx#NonFungibleTokenModule-_mint) without some sort of access control, for example, would allow ANYONE to mint tokens.
