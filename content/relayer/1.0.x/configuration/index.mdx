...
| `REDIS_URL` | `redis://localhost:6379` | `<redis connection string>` | Redis connection URL for the relayer. |
| `REDIS_CONNECTION_TIMEOUT_MS` | `10000` | `<timeout in milliseconds>` | Connection timeout for Redis in milliseconds. |
...
See the dedicated [Signers Configuration](/relayer/1.0.x/configuration/signers) guide.
...
| network | String | Network the relayer will connect to. Must match a network identifier defined in your network configuration files. See [Network Configuration](/relayer/1.0.x/network_configuration) for details on defining networks. |
...
| policies | list | Overrides default policies. Please refer to the [`Policies`](/relayer/1.0.x/configuration#network-policies) table |
...
For more information on how to write a plugin, please refer to the [Plugins](/relayer/1.0.x/plugins) page.
...
See the dedicated [Network Configuration](/relayer/1.0.x/network_configuration) guide.
