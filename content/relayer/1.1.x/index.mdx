...
For detailed network configuration options and examples, see the [Network Configuration](/relayer/network-configuration) page.
...
<Callout>
Before executing the command, ensure that the `.env` and `config.json` files are configured as detailed in the [Configuration References](/relayer/configuration) section.
</Callout>
