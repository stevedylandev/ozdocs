		...
		const viewport = (
			<SidebarViewport>
				{links
					...
					.map((item, i, arr) => (
						<SidebarLinkItem
							key={"url" in item ? item.url : `${item.type}-${i}`}
							item={item}
		...
		return (
			<Popover>
				...
				<PopoverContent className="flex flex-col">
					{item.items.map((child, _i) => {
						// Create a stable key using item properties, avoiding array index
						const key = String(
							("url" in child && child.url) ||
								("text" in child && child.text) ||
								`menu-item-${child.type}`,
						);
						if (child.type === "custom")
							return (
								<Fragment key={`custom-${key}`}>{child.children}</Fragment>
							);
