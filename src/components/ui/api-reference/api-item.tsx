...
export async function APIItem({ functionName, kind, id, children }: Props) {
	const anchorId =
		id ?? `api-${functionName.replace(/\W+/g, "-").toLowerCase()}`;

	return (
		<div id={anchorId}>
			<div className="border rounded-md mb-4">
				<div className="bg-secondary flex w-full justify-between px-4">
					<p className="font-bold text-sm font-mono">{functionName}</p>
